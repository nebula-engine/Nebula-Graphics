...
void main(void)
{
	...
	{
		// ambient
		ambient = lights_point[i].ambient * front.ambient * image_color;
		...
		diffuse = atten * lights_point[i].diffuse * front.diffuse * image_color * vec4(vec3(angle),1.0);
	...
	for(int i = 0; i < light_count_spot; i++) // for all light sources
	{
		...
		// ambient
		ambient = lights_spot[i].ambient * front.ambient * image_color;
		...
		diffuse = atten * lights_spot[i].diffuse * front.diffuse * image_color * vec4(vec3(angle),1.0);
